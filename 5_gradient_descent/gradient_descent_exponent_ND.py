...
def polynomial_regression(X_array, Y_array, weights, iterations=2000):
...
def exponent_regrision(X_array, Y_array):
	...
	teta = polynomial_regression(X_array, log_y_array, weights_coordinates, iterations)
...
def exp_function(array, teta):
	...
	for j in range(len(array)):
		for i in range(len(teta)):
			result[j] += math.exp(teta[i] * array[j]**i)
	return result
